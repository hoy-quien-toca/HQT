  ...
  return (
    <div className="min-h-screen text-white font-sans relative overflow-x-hidden">
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-6 space-y-12 relative z-10">
        ...
        {featuredEvents.length > 0 && (
          <section className="relative h-[400px] md:h-[500px] border-4 md:border-8 border-white bg-zinc-800 flex items-end p-6 md:p-10 overflow-hidden shadow-[12px_12px_0px_0px_rgba(234,179,8,1)] group">
            ...
            <div className="relative z-20 w-full text-left">
              <span className="bg-red-600 text-white px-4 py-1 text-xs md:text-sm font-black uppercase italic tracking-widest shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
        ...
        <div className="flex flex-col lg:flex-row gap-8 md:gap-12 text-left">
          <div className="flex-1 space-y-12">
            ...
            <section className="bg-yellow-400 text-black p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 items-center font-black uppercase italic shadow-[8px_8px_0px_0px_rgba(255,255,255,1)]">
              <select value={department} onChange={(e) => { setDepartment(e.target.value); applyFilters(allEvents, e.target.value, genre, ageRating, priceType); }} className="bg-black text-white p-2 border-2 border-white focus:outline-none font-bold text-xs">
                <option value="">Depto</option>
                {activeDepartments.map(d => <option key={d} value={d}>{d}</option>)}
              </select>
              <select value={genre} onChange={(e) => { setGenre(e.target.value); applyFilters(allEvents, department, e.target.value, ageRating, priceType); }} className="bg-black text-white p-2 border-2 border-white focus:outline-none font-bold text-xs">
              ...
              <select value={priceType} onChange={(e) => { setPriceType(e.target.value); applyFilters(allEvents, department, genre, ageRating, e.target.value); }} className="bg-black text-white p-2 border-2 border-white focus:outline-none font-bold text-xs">
          ...
          <aside className="lg:w-72 space-y-8 relative z-10 text-left">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-1 gap-4 lg:space-y-6">
              {sidebarSponsors.map(ad => (
                <div key={ad.id} onClick={() => setSelectedAd(ad)} className="block border-4 border-white bg-zinc-950 p-2 shadow-[6px_6px_0px_0px_rgba(234,179,8,1)] hover:-translate-x-1 transition-transform group cursor-pointer">
                  <div className="aspect-[4/5] overflow-hidden border-2 border-zinc-800">
                    <img src={ad.image_url} alt="Sponsor" className="w-full h-full object-cover" />
                  </div>
                </div>
              ))}
