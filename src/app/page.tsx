  ...
  return (
    <div className="min-h-screen bg-zinc-900 text-white font-sans relative overflow-x-hidden">
      ...
      <header className="border-b-4 border-yellow-400 p-6 flex justify-between items-center bg-zinc-950 sticky top-0 z-50 shadow-xl">
        ...
        <nav className="hidden md:flex gap-6 font-bold uppercase tracking-widest text-sm relative z-10">
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-12 relative z-10">
        {topAd && (
          <a href={topAd.link} target="_blank" rel="noopener noreferrer" className="block w-full h-24 bg-zinc-950 border-4 border-white overflow-hidden shadow-[8px_8px_0px_0px_rgba(255,255,255,1)]">
            <img src={topAd.image_url} alt="Publicidad" className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all" />
          </a>
        )}
        ...
        {featuredEvent && (
          <section className="relative h-[400px] border-8 border-white bg-zinc-800 flex items-end p-8 overflow-hidden shadow-[12px_12px_0px_0px_rgba(234,179,8,1)]">
            ...
            <div className="relative z-20">
              <span className="bg-red-600 text-white px-3 py-1 text-sm font-black uppercase italic tracking-widest shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">{featuredEvent.suggestion_tag || 'Planazo'}</span>
              <h2 className="text-5xl md:text-7xl font-black uppercase italic leading-[0.85] tracking-tighter mt-2">{featuredEvent.band_name}</h2>
              <p className="text-xl font-bold text-yellow-400 uppercase tracking-widest border-l-4 border-yellow-400 pl-3">{featuredEvent.date} @ {featuredEvent.venue}</p>
            </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          <div className="flex-1 space-y-12">
            <section className="bg-yellow-400 text-black p-4 flex flex-wrap gap-4 items-center font-black uppercase italic shadow-[8px_8px_0px_0px_rgba(255,255,255,1)]">
              ...
              <select onChange={(e) => setDepartment(e.target.value)} className="bg-black text-white p-2 border-2 border-white focus:outline-none font-bold">
              ...
              <button onClick={fetchData} className="bg-red-600 text-white px-6 py-2 border-2 border-white hover:bg-black transition-colors font-black">Buscar</button>
            </section>
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {loading ? (
                ...
                <div className="col-span-full text-center py-20 space-y-4 border-4 border-dashed border-zinc-700">
                  ...
                  <Link href="/submit" className="inline-block bg-white text-black px-8 py-3 font-black uppercase hover:bg-yellow-400 transition-colors shadow-[4px_4px_0px_0px_rgba(234,179,8,1)]">Subir Fecha</Link>
                </div>
              ) : (
                events.map((event) => {
                  return (
        ...
        <footer className="border-t-4 border-zinc-800 pt-8 pb-16 text-center">
          <p className="text-zinc-500 font-bold uppercase text-[10px] max-w-2xl mx-auto tracking-tighter leading-relaxed">AVISO: HOY QUIEN TOCA NO VENDE ENTRADAS. SOMOS UNA PLATAFORMA INFORMATIVA. LA VENTA Y ORGANIZACIÓN ES RESPONSABILIDAD DE LOS ORGANIZADORES.</p>
        </footer>
      </main>
    </div>
  );
