...
export default function Home() {
  ...
  function applyFilters(dep: string, gen: string, d: string) {
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-12 relative z-10">
        {topAd && (
          <a href={topAd.link} target="_blank" rel="noopener noreferrer" className="block w-full h-24 bg-zinc-950 border-4 border-white overflow-hidden shadow-[8px_8px_0px_0px_rgba(255,255,255,1)] group">
            <img src={topAd.image_url} alt="Ad" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all" />
          </a>
        )}
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          <div className="flex-1 space-y-12">
            <section className="bg-yellow-400 text-black p-4 flex flex-wrap gap-4 items-center font-black uppercase italic shadow-[8px_8px_0px_0px_rgba(255,255,255,1)]">
              ...
              <select value={genre} onChange={(e) => { setGenre(e.target.value); applyFilters(department, e.target.value, date); }} className="bg-black text-white p-2 border-2 border-white focus:outline-none font-bold min-w-[150px]">
              ...
              {loading ? (
                ...
                events.map((event) => (
                  <div key={event.id} onClick={() => setSelectedEvent(event)} className="border-4 border-white p-4 hover:translate-x-2 hover:-translate-y-2 transition-all bg-zinc-950 shadow-[8px_8px_0px_0px_rgba(234,179,8,1)] flex flex-col group/card relative overflow-hidden cursor-pointer">
                    ...
                    {event.is_sold_out && <div className="absolute top-8 -right-12 bg-red-600 text-white font-black py-2 px-12 rotate-45 uppercase text-sm border-y-2 border-white z-20 shadow-xl tracking-tighter italic">¡AGOTADO!</div>}
                    <div className="aspect-square bg-zinc-800 mb-4 border-2 border-zinc-700 flex items-center justify-center overflow-hidden">
                      {event.flyer_url ? <img src={event.flyer_url} alt="Flyer" className={`object-cover w-full h-full grayscale group-hover/card:grayscale-0 transition-all duration-500 ${event.is_sold_out ? 'blur-[1px]' : ''}`} /> : <div className="text-zinc-600 font-black italic">FLYER DEL SHOW</div>}
                    </div>
        ...
        {selectedEvent && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10">
            ...
            <div className="relative w-full max-w-4xl bg-zinc-900 border-8 border-white shadow-[20px_20px_0px_0px_rgba(234,179,8,1)] flex flex-col md:flex-row overflow-y-auto max-h-full">
              ...
              <div className="md:w-1/2 p-8 space-y-6 text-left">
                <div>
                ...
                <div className="border-t-2 border-zinc-800 pt-6">
                  ...
                  <div className="text-zinc-300 leading-relaxed font-medium space-y-4 max-h-48 overflow-y-auto pr-4 scrollbar-thin scrollbar-thumb-yellow-400 uppercase text-sm">
        ...
        <footer className="border-t-4 border-zinc-800 pt-8 pb-16 text-center">
          <p className="text-zinc-500 font-bold uppercase text-[10px] max-w-2xl mx-auto tracking-tighter leading-relaxed italic italic">AVISO: HOY QUIEN TOCA NO VENDE ENTRADAS. SOMOS UNA PLATAFORMA INFORMATIVA. LA VENTA Y ORGANIZACIÓN ES RESPONSABILIDAD DE LOS ORGANIZADORES.</p>
        </footer>
      </main>
    </div>
  );
