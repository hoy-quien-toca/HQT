...
export default function Home() {
  ...
  return (
    <div className="min-h-screen text-white font-sans relative overflow-x-hidden text-left bg-zinc-800">
      ...
      <header className="border-b-4 border-blue-500 p-4 md:p-6 bg-zinc-950 sticky top-0 z-50 shadow-xl">
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-6 space-y-12 relative z-10">
        ...
        {topSponsor && (
          <div onClick={() => setSelectedAd(topSponsor)} className="cursor-pointer block w-full h-24 bg-zinc-950 border-4 border-white overflow-hidden shadow-lg group relative rounded-3xl">
            ...
            <div className="absolute top-2 left-2 bg-black/60 text-white text-[8px] font-black px-2 py-0.5 border border-blue-500 uppercase tracking-widest rounded-lg">Publicidad</div>
          </div>
        )}
        ...
        {featuredEvents.length > 0 && (
          <section className="relative h-[400px] md:h-[500px] border-4 md:border-8 border-white bg-zinc-800 flex items-end p-6 md:p-10 overflow-hidden shadow-[12px_12px_0px_0px_rgba(59,130,246,0.3)] group rounded-[40px]">
            ...
            <div className="relative z-20 w-full">
              ...
              <p className="text-lg md:text-2xl font-bold text-white uppercase tracking-widest border-l-4 md:border-l-8 border-blue-500 pl-4 mt-4">
                {featuredEvents[currentHeroIndex].date} @ {featuredEvents[currentHeroIndex].venue}
              </p>
              <button onClick={() => setSelectedEvent(featuredEvents[currentHeroIndex])} className="mt-8 bg-white text-black font-black uppercase px-6 py-2 md:px-8 md:py-3 hover:bg-blue-500 hover:text-white transition-all text-sm md:text-base rounded-full shadow-lg">Ver Detalles</button>
            </div>
            <div className="absolute top-6 right-8 z-30 flex gap-1 md:gap-2">
              {featuredEvents.map((_, i) => (
                <div key={i} className={`h-1.5 md:h-2 w-6 md:w-8 border border-white transition-all rounded-full ${i === currentHeroIndex ? 'bg-blue-500 w-10 md:w-12' : 'bg-transparent opacity-50'}`} />
              ))}
            </div>
          </section>
        )}
        ...
        <div className="flex flex-col lg:flex-row gap-8 md:gap-12">
          <div className="flex-1 space-y-12 text-left">
            ...
            <section className="bg-blue-500 text-black p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 items-center font-black uppercase italic shadow-[8px_8px_0px_0px_rgba(255,255,255,1)] rounded-3xl">
              ...
              <button onClick={() => {setDepartment(''); setGenre(''); setAgeRating(''); setPriceType(''); setEvents(allEvents);}} className="sm:col-span-full text-[10px] underline hover:text-white font-bold uppercase tracking-widest text-center mt-2">Limpiar Filtros</button>
            </section>
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
              {loading ? (
                ...
                events.map((event) => (
                  <div key={event.id} onClick={() => setSelectedEvent(event)} className="border-4 border-white p-4 hover:translate-x-1 hover:-translate-y-1 transition-all bg-zinc-950 shadow-[8px_8px_0px_0px_rgba(59,130,246,0.5)] flex flex-col group/card relative overflow-hidden cursor-pointer rounded-[32px]">
                    <div className="absolute top-2 left-2 flex flex-col gap-1 z-20">
                      {event.suggestion_tag && <div className="bg-blue-500 text-white text-[8px] font-black px-2 py-0.5 shadow-md uppercase rounded-sm">{event.suggestion_tag}</div>}
        ...
        {activeBottomAd && (
          <section className="pt-12">
             <div onClick={() => setSelectedAd(activeBottomAd)} className="cursor-pointer block w-full h-48 md:h-64 bg-zinc-950 border-4 md:border-8 border-white overflow-hidden shadow-[12px_12px_0px_0px_rgba(59,130,246,0.5)] group relative rounded-[40px]">
                ...
                <div className="absolute top-4 left-4 bg-black/80 text-white text-[10px] font-black px-4 py-1 border-2 border-blue-500 uppercase tracking-widest rounded-full italic">Auspiciante Destacado</div>
             </div>
        </section>
        )}
        ...
        {selectedEvent && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-4xl bg-zinc-900 border-4 md:border-8 border-white shadow-[20px_20px_0px_0px_rgba(59,130,246,0.5)] flex flex-col md:flex-row overflow-y-auto max-h-[90vh] rounded-[40px]">
              ...
              <div className="md:w-1/2 p-6 md:p-8 space-y-6 text-left">
                ...
                <div className="pt-6 border-t-2 border-zinc-800 space-y-4">
                  <p className="text-2xl md:text-3xl font-black italic text-blue-500 tracking-tighter uppercase">
        ...
        {selectedAd && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative max-w-2xl w-full bg-zinc-900 border-8 border-white p-4 shadow-[20px_20px_0px_0px_rgba(59,130,246,0.3)] text-center rounded-[40px]">
