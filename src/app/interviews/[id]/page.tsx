    ...
    <div className="min-h-screen text-white font-sans relative overflow-x-hidden text-left bg-zinc-900">
        ...
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-[8px] md:text-[10px] font-bold text-white uppercase tracking-widest mt-1 font-black">Descubri recitales, toques y eventos musicales en tu Ciudad</p>
            </div>
          </div>
          ...
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="md:hidden text-red-600">
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24 font-black"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={isMenuOpen ? "M6 18L18 6M6 6l12 12" : "M4 6h16M4 12h16M4 18h16"}></path></svg>
          </button>
        </div>
      </header>
      ...
      <article className="max-w-4xl mx-auto p-6 space-y-8 py-16 relative z-10">
        <div className="space-y-4 text-center">
          <span className="bg-red-600 text-white px-4 py-1 text-sm font-franklin italic tracking-widest shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] rounded-full">
            BANDA: {interview.band_name}
          </span>
          <h1 className="text-4xl md:text-6xl font-franklin tracking-tighter text-white leading-none">
            {interview.title}
          </h1>
          <div className="flex flex-col md:flex-row justify-center items-center gap-4 pt-4">
             ...
             {interview.author && (
               <p className="text-red-600 font-franklin uppercase text-xs italic tracking-widest">
        ...
        {interview.image_url && (
          <div className="flex justify-center">
            <div className="border-8 border-white shadow-[12px_12px_0px_0px_rgba(220,38,38,0.3)] overflow-hidden relative group rounded-[40px] max-w-3xl w-full">
              <img src={interview.image_url} alt={interview.band_name} className="w-full h-auto grayscale group-hover:grayscale-0 transition-all duration-700" />
        ...
        <div className="prose prose-invert max-w-none text-xl md:text-2xl leading-relaxed font-bold space-y-6 text-zinc-200 uppercase tracking-tight font-black pt-8">
          {interview.content.split('\n').map((para: string, i: number) => (
            <p key={i}>{para}</p>
          ))}
        </div>

        <div className="pt-16 border-t-4 border-zinc-800 flex flex-col items-center gap-8 font-black">
          <p className="text-zinc-500 font-black uppercase text-xs italic">Gracias por leer Hoy Quien Toca</p>
          <button onClick={shareOnWhatsApp} className="bg-green-600 text-white px-10 py-4 font-black uppercase text-lg hover:bg-white hover:text-green-600 shadow-[8px_8px_0px_0px_rgba(255,255,255,1)] transition-all rounded-full border-4 border-black italic tracking-tighter font-franklin">Compartir en WhatsApp</button>
        </div>
      </article>
