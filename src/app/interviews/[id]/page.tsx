...
export default function InterviewDetail() {
  ...
  useEffect(() => {
    if (interviewId) {
      ...
      // Small delay to allow params to hydrate on some mobile browsers
  ...
  async function fetchData(id: string) {
    try {
      ...
      if (data) {
        ...
        setErrorInfo("Entrevista no encontrada.");
      }
    } catch (err: any) {
      console.error("Load failed:", err);
      setErrorInfo(err.message || "Error de conexión");
    } finally {
      setLoading(false);
    }
  }
  ...
  if (errorInfo || !interview) {
    return (
      <div className="min-h-screen bg-zinc-900 text-white flex flex-col items-center justify-center p-6 text-center font-black">
        <h1 className="text-6xl uppercase italic text-red-600 mb-4 tracking-tighter font-franklin">¡Ups!</h1>
        <p className="text-xl uppercase tracking-widest mb-8">{errorInfo || "No encontramos lo que buscas."}</p>
        <div className="flex gap-4">
           <button onClick={() => window.location.reload()} className="bg-red-600 text-white px-8 py-3 hover:bg-white hover:text-black transition-colors uppercase rounded-full font-black">Reintentar</button>
           <Link href="/interviews" className="bg-white text-black px-8 py-3 hover:bg-red-600 hover:text-white transition-colors uppercase rounded-full font-black">Volver</Link>
        </div>
      </div>
    );
  }
      ...
      <header className="border-b-4 border-red-600 p-4 md:p-6 bg-zinc-950 sticky top-0 z-50 shadow-xl">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-6 font-bold uppercase text-sm items-center">
            ...
            <Link href="/submit" className="border-2 border-red-600 text-red-600 px-4 py-1 bg-black rounded-full animate-pulse">Subir Fecha</Link>
          </nav>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-6 py-10 relative z-10">
        ...
        <article className="space-y-8 font-black">
          <div className="text-center space-y-4">
            <span className="bg-red-600 text-white px-4 py-1 text-xs md:text-sm font-franklin italic rounded-full inline-block">BANDA: {interview.band_name}</span>
          ...
          {interview.image_url && (
            <div className="flex justify-center">
              <div className="border-4 md:border-8 border-white shadow-xl overflow-hidden relative group rounded-[24px] md:rounded-[40px] w-full">
                <img 
                ...
                />
                {interview.photo_credit && <div className="absolute bottom-2 right-2 bg-black/80 text-white text-[8px] font-black px-2 py-1 uppercase rounded-full">Foto: {interview.photo_credit}</div>}
              </div>
            </div>
          )}

          <div className="max-w-none text-lg md:text-2xl leading-relaxed font-bold space-y-6 text-zinc-200 uppercase tracking-tight pt-8 whitespace-pre-wrap">
            {interview.content}
          </div>

          <div className="pt-16 border-t-4 border-zinc-800 flex flex-col items-center gap-8 pb-24">
      ...
      {showLogoModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative max-w-lg w-full bg-zinc-900 border-8 border-white p-6 rounded-[50px] shadow-2xl text-center">
            ...
            <h3 className="text-3xl font-franklin text-red-600 mt-4">Hoy Quien Toca</h3>
          </div>
        </div>
      )}
    </div>
