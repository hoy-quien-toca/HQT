    ...
    <div className="min-h-screen text-white font-sans relative overflow-x-hidden bg-zinc-900">
      ...
      <header className="border-b-4 border-red-600 p-4 md:p-6 bg-zinc-950 sticky top-0 z-50 shadow-xl text-left">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-6 font-black">
            ...
            <div>
              <h1 className="text-2xl md:text-5xl font-franklin tracking-tighter text-red-600 leading-none font-black">Hoy Quien Toca</h1>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[60] bg-black/95 flex flex-col items-center justify-center space-y-8 md:hidden text-center font-black">
          ...
          <Link href="/" onClick={() => setIsMenuOpen(false)} className="text-4xl uppercase text-white italic font-franklin font-black">Fechas</Link>
          <Link href="/interviews" onClick={() => setIsMenuOpen(false)} className="text-4xl uppercase text-red-600 italic font-franklin font-black">Entrevistas</Link>
          <Link href="/contact" onClick={() => setIsMenuOpen(false)} className="text-4xl uppercase text-white italic font-franklin font-black">Contacto</Link>
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-6 space-y-12 relative z-10 font-black">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 font-black">
            <p className="text-3xl font-black uppercase italic text-zinc-600 tracking-tighter font-black font-black font-black">Próximamente nuevas entrevistas...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-left font-black font-black">
                ...
                <div className="aspect-video bg-zinc-800 mb-6 border-2 border-zinc-700 overflow-hidden rounded-2xl font-black">
                  {interview.image_url ? (
                    <img 
                      ...
                      alt={interview.title} 
                      className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-500 font-black font-black"
                      style={{ objectPosition: interview.image_position || 'center' }} 
                    />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-zinc-500 font-black italic uppercase text-center font-black font-black">Sin Imagen</div>
                  )}
                </div>
                <div className="space-y-3 font-black font-black">
                  <span className="bg-red-600 text-white px-3 py-1 text-xs font-black uppercase tracking-widest italic rounded-full font-black font-black">BANDA: {interview.band_name}</span>
                  <h3 className="text-3xl font-franklin leading-none group-hover:text-red-600 transition-colors text-white font-black font-black font-black">{interview.title}</h3>
                  {interview.subtitle && (
                    <p className="text-zinc-400 text-sm font-bold line-clamp-2 uppercase font-black font-black">{interview.subtitle}</p>
                  )}
                  <div className="flex justify-between items-center text-zinc-500 font-bold text-[10px] uppercase tracking-tighter font-black font-black font-black">
