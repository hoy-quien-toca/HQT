    ...
    <div className="min-h-screen bg-zinc-900 text-white p-4 md:p-6 font-sans relative text-left overflow-x-hidden font-black">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-10 font-black">
        ...
        <section className="space-y-6 text-left">
          <div className="flex justify-between items-center bg-zinc-950 p-4 border-l-8 border-red-600 rounded-r-2xl">
             ...
             <button onClick={() => setEditingEvent({ id: 'new', band_name: '', venue: '', address: '', city: '', department: 'MONTEVIDEO', date: '', time: '21:00', age_rating: 'ATP', description: '', is_approved: false, price_type: 'free', genre: 'ROCK', flyer_url: '' })} className="bg-red-600 text-white px-6 py-2 text-[10px] font-black uppercase rounded-full hover:bg-white hover:text-black transition-all shadow-lg font-black font-black font-black">+ NUEVA FECHA</button>
          </div>
          ...
          {editingEvent && (
            <div className="border-4 border-blue-600 p-4 bg-zinc-950 space-y-4 mb-8 shadow-lg rounded-3xl font-black font-black">
              ...
              <form onSubmit={handleSaveEvent} className="grid grid-cols-2 gap-3 text-xs text-white font-black font-black font-black">
                ...
                <div className="col-span-2 flex gap-2 font-black font-black font-black">
                  <button type="submit" className="flex-1 bg-blue-600 py-3 font-black border-2 border-white rounded-full uppercase shadow-lg font-black">GUARDAR FECHA</button>
          ...
          <div className="space-y-4 font-black">
            {events.map((event) => (
              <div key={event.id} className={`border-4 p-4 flex flex-col gap-4 ${!event.is_approved ? 'border-red-600 bg-zinc-900 animate-pulse shadow-lg' : event.is_featured ? 'border-red-600 bg-zinc-950/80 shadow-md' : 'border-zinc-700 bg-zinc-950/80'} rounded-[32px] font-black font-black`}>
                ...
                {event.is_approved && (
                  <div className="space-y-3 border-t border-zinc-800 pt-3 font-black font-black">
                    ...
                    <div className="flex flex-wrap gap-2 items-center font-black font-black">
                      ...
                      <button onClick={() => updateEventTag(event.id, 'SALIDA SEGURA')} className={`px-2 py-1 text-[8px] font-black border-2 rounded-full ${event.suggestion_tag === 'SALIDA SEGURA' ? 'bg-green-600 border-white text-white shadow-sm' : 'border-green-600 text-green-600 font-black font-black'}`}>SEGURA</button>
                      <button onClick={() => updateEventTag(event.id, 'NO FALLA')} className={`px-2 py-1 text-[8px] font-black border-2 rounded-full ${event.suggestion_tag === 'NO FALLA' ? 'bg-white border-black text-black shadow-sm' : 'border-white text-white font-black font-black'}`}>NO FALLA</button>
                      <button onClick={() => updateEventTag(event.id, '')} className="text-[8px] font-black uppercase text-red-600 underline ml-auto italic font-black font-black font-black font-black">X TAG</button>
                    </div>
                  </div>
                )}
              </div>
        ...
        <section className="space-y-12 font-black font-black">
          ...
          <div className="space-y-6 text-left font-black font-black">
            <h2 className="text-2xl font-black uppercase italic text-red-600 border-l-8 border-red-600 pl-4 bg-zinc-950 py-2 font-franklin font-black font-black">Publicidad</h2>
          ...
          <div className="space-y-6 border-t-4 border-zinc-800 pt-8 text-left font-black">
            <h2 className="text-2xl font-black uppercase italic text-red-600 border-l-8 border-red-600 pl-4 bg-zinc-950 py-2 font-franklin font-black">Entrevistas</h2>
            <form onSubmit={handleSaveInterview} className="bg-zinc-950 p-4 border-4 border-white space-y-4 shadow-xl rounded-[32px] font-black font-black">
              <input placeholder="Título" className="w-full bg-black border-2 border-white p-2 font-bold uppercase text-xs text-white focus:border-red-600 outline-none rounded-xl font-black" value={newInterview.title} onChange={e => setNewInterview({...newInterview, title: e.target.value})} required />
              <input placeholder="Banda" className="w-full bg-black border-2 border-white p-2 font-bold uppercase text-xs text-white focus:border-red-600 outline-none rounded-xl font-black" value={newInterview.band_name} onChange={e => setNewInterview({...newInterview, band_name: e.target.value})} required />
          ...
          {/* Mensajes */}
          <div className="space-y-6 border-t-4 border-zinc-800 pt-8 text-left text-white font-black font-black">
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`border-2 p-3 flex justify-between items-center ${msg.is_read ? 'border-zinc-800 bg-zinc-950/50 opacity-60' : 'border-white bg-zinc-900'} rounded-2xl font-black`}>
                  <div onClick={() => setSelectedMessage(msg)} className="cursor-pointer flex-1 font-black">
                    ...
                    <p className="text-[9px] text-zinc-300 truncate max-w-[150px] font-black font-black">"{msg.message}"</p>
                  </div>
                  <button onClick={() => deleteMessage(msg.id)} className="text-[7px] bg-red-600 text-white px-2 py-0.5 border border-white rounded-full font-black">BORRAR</button>
                </div>
              ))}
            </div>
          </div>
      ...
      {selectedMessage && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 font-black">
          ...
          <div className="relative w-full max-w-xl bg-zinc-900 border-8 border-white p-10 shadow-2xl rounded-[50px] font-black text-left">
