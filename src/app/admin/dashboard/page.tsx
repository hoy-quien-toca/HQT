              ...
              <div key={event.id} className={`border-4 p-4 flex flex-col gap-4 ${!event.is_approved ? 'border-red-600 bg-zinc-900 animate-pulse shadow-lg' : event.is_featured ? 'border-yellow-400 bg-zinc-950/80 shadow-md' : 'border-zinc-700 bg-zinc-950/80'}`}>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  ...
                  <div className="flex gap-2 w-full md:w-auto">
                    ...
                    <button onClick={() => deleteEvent(event.id)} className="bg-red-600 text-white px-3 py-1 font-black text-[10px] border-2 border-white shadow-sm font-black">ELIMINAR</button>
                  </div>
                </div>
                ...
                {event.is_approved && (
                  <div className="space-y-3 border-t border-zinc-800 pt-3">
                    ...
                    <button onClick={() => toggleSoldOut(event.id, event.is_sold_out)} className={`w-full py-1 text-[9px] font-black border-2 ${event.is_sold_out ? 'bg-red-600 border-white text-white shadow-lg animate-pulse' : 'border-zinc-700 text-zinc-500'}`}>{event.is_sold_out ? 'VENDER DE NUEVO' : 'MARCAR AGOTADO'}</button>
                  </div>
                )}
              </div>
            ))}
