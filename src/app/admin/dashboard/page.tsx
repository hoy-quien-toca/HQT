      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-10">
        ...
        <section className="space-y-6 text-left">
          <div className="flex justify-between items-center bg-zinc-950 p-3 border-l-4 border-red-600 rounded-r-xl">
             ...
             <button onClick={() => setEditingEvent({ id: 'new', band_name: '', venue: '', address: '', city: '', department: 'MONTEVIDEO', date: '', time: '21:00', age_rating: 'ATP', description: '', is_approved: false, price_type: 'free', genre: 'ROCK', flyer_url: '' })} className="bg-red-600 text-white px-4 py-1.5 text-[9px] font-black uppercase rounded-full border-2 border-white">+ NUEVA FECHA</button>
          </div>
          ...
          {editingEvent && (
            <div className="border-4 border-blue-600 p-4 bg-zinc-950 space-y-4 mb-8 shadow-lg rounded-3xl font-black">
              ...
              <form onSubmit={handleSaveEvent} className="grid grid-cols-2 gap-3 text-[10px] text-white">
                ...
                <input value={editingEvent.venue || ''} onChange={e => setEditingEvent({...editingEvent, venue: e.target.value})} className="bg-black border-2 border-white p-2 uppercase rounded-lg font-black" placeholder="Lugar" />
                <input value={editingEvent.city || ''} onChange={e => setEditingEvent({...editingEvent, city: e.target.value})} className="bg-black border-2 border-white p-2 uppercase rounded-lg font-black" placeholder="Ciudad" />
                
                <select value={editingEvent.department} onChange={e => setEditingEvent({...editingEvent, department: e.target.value})} className="bg-black border-2 border-white p-2 rounded-lg font-black">
                  {DEPARTAMENTOS.map(d => <option key={d} value={d}>{d}</option>)}
                </select>
                <select value={editingEvent.genre} onChange={e => setEditingEvent({...editingEvent, genre: e.target.value})} className="bg-black border-2 border-white p-2 rounded-lg font-black">
                ...
                <div className="col-span-2 space-y-1">
                   ...
                   <div className="flex gap-4 items-center border-2 border-dashed border-zinc-700 p-2 relative rounded-xl font-black">
                      <p className="text-[9px] font-black uppercase text-zinc-500 flex-1">{uploading ? 'Cargando...' : (editingEvent.flyer_url ? 'Imagen OK ✅' : 'Clic para elegir imagen')}</p>
                ...
                <div className="col-span-2 flex gap-2 font-black">
                  ...
                  <button type="button" onClick={() => setEditingEvent(null)} className="bg-zinc-700 px-6 font-black border border-white rounded-full">X</button>
                </div>
              </form>
            </div>
          )}
        ...
        <section className="space-y-12 font-black">
          ...
          <div className="space-y-6 font-black">
            <div className="flex justify-between items-center bg-zinc-950 p-4 border-l-8 border-red-600 rounded-r-2xl font-black font-black">
               <h2 className="text-3xl font-black uppercase italic text-red-600 font-franklin">Entrevistas Hechas</h2>
            </div>
            <div className="grid grid-cols-1 gap-6 max-h-[600px] overflow-y-auto pr-4 custom-scrollbar">
              {interviews.map(int => (
                <div key={int.id} className={`border-4 p-8 flex justify-between items-center shadow-2xl transition-all ${int.is_active ? 'border-red-600 bg-zinc-900 scale-105' : 'border-zinc-800 bg-zinc-950 opacity-40 grayscale italic'} rounded-[50px] font-black font-black`}>
                  ...
                  <div className="flex flex-col gap-2 shrink-0 font-black">
                    ...
                    <button onClick={() => setNewInterview(int)} className="px-8 py-2 bg-blue-600 text-white text-[10px] font-black border-2 border-white rounded-full hover:bg-white hover:text-black uppercase font-black">EDITAR</button>
                    <button onClick={() => confirm('¿Borrar?') && deleteInterview(int.id)} className="px-8 py-2 bg-red-600 text-white text-[10px] font-black border-2 border-white rounded-full uppercase font-black">ELIMINAR</button>
                  </div>
                </div>
              ))}
            </div>
