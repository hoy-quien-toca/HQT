...
export default function AdminDashboard() {
  ...
  async function handleCreateAd(e: React.FormEvent) {
    e.preventDefault();
    if (!newAd.image_url) { alert('Sube una imagen primero'); return; }
  ...
  async function handleCreateInterview(e: React.FormEvent) {
    e.preventDefault();
    if (!newInterview.image_url) { alert('Sube una foto primero'); return; }
    ...
    <div className="min-h-screen bg-zinc-900 text-white p-6 font-sans">
      <header className="flex justify-between items-center mb-12 border-b-4 border-yellow-400 pb-6 bg-zinc-950 p-4 sticky top-0 z-50">
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => supabase.auth.signOut().then(() => router.push('/admin'))} className="bg-red-600 px-4 py-2 font-black uppercase text-sm">Cerrar Sesión</button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
            {events.map((event) => (
              <div key={event.id} className={`border-4 p-4 flex flex-col gap-4 ${event.is_approved ? 'border-zinc-700 bg-zinc-950 opacity-80' : 'border-white bg-zinc-800'}`}>
                <div className="flex justify-between items-center w-full">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => deleteEvent(event.id)} className="bg-red-600 px-3 py-1 font-black uppercase text-[10px]">Borrar</button>
                  </div>
                </div>

                {event.is_approved && (
                  <div className="flex flex-wrap gap-2 border-t border-zinc-800 pt-4">
                    <button onClick={() => updateEventTag(event.id, 'PLANAZO')} className={`px-2 py-1 text-[9px] font-black uppercase border-2 ${event.suggestion_tag === 'PLANAZO' ? 'bg-red-600 border-white' : 'border-red-600 text-red-600'}`}>PLANAZO</button>
                    ...
                    <button onClick={() => updateEventTag(event.id, '')} className="px-2 py-1 text-[9px] font-black uppercase text-zinc-500 underline">Quitar</button>
                    <button onClick={() => toggleSoldOut(event.id, event.is_sold_out)} className={`ml-auto px-4 py-1 text-[9px] font-black uppercase border-2 ${event.is_sold_out ? 'bg-red-600 border-white text-white' : 'border-white text-white'}`}>
        ...
        <section className="space-y-6 border-t-4 border-zinc-800 pt-12 lg:col-span-2">
          ...
          <form onSubmit={handleCreateAd} className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-zinc-950 p-6 border-4 border-white mb-8">
            <div className="space-y-4">
              <input placeholder="Nombre del Cliente" className="w-full bg-black border-2 border-white p-3 font-bold uppercase text-sm text-white outline-none focus:border-yellow-400" value={newAd.client_name} onChange={e => setNewAd({...newAd, client_name: e.target.value})} required />
              <div className="space-y-2 border-2 border-dashed border-zinc-700 p-4 text-center relative hover:border-yellow-400 transition-colors">
                <p className="text-xs font-black uppercase text-zinc-500">{uploading ? 'Subiendo...' : (newAd.image_url ? 'Imagen Lista ✅' : 'Subir Imagen/GIF/Video')}</p>
            ...
            <div className="space-y-4">
              <input placeholder="Link de Destino" className="w-full bg-black border-2 border-white p-3 font-bold uppercase text-sm text-white outline-none focus:border-yellow-400" value={newAd.link} onChange={e => setNewAd({...newAd, link: e.target.value})} />
              <select className="w-full bg-black border-2 border-white p-3 font-bold uppercase text-sm text-white outline-none" value={newAd.position} onChange={e => setNewAd({...newAd, position: e.target.value})}>
                <option value="sidebar">Barra Lateral</option>
                <option value="top">Banner Superior</option>
              </select>
              <button type="submit" disabled={uploading} className="w-full bg-yellow-400 text-black font-black uppercase py-3 hover:bg-white disabled:opacity-50">Guardar Publicidad</button>
            </div>
          </form>
        </section>
        ...
        <section className="space-y-6 border-t-4 border-zinc-800 pt-12 lg:col-span-2 pb-20">
          ...
          <form onSubmit={handleCreateInterview} className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-zinc-950 p-6 border-4 border-white mb-8">
            <div className="space-y-4">
              <input placeholder="Título" className="w-full bg-black border-2 border-white p-3 font-bold uppercase text-sm text-white outline-none focus:border-yellow-400" value={newInterview.title} onChange={e => setNewInterview({...newInterview, title: e.target.value})} required />
              <input placeholder="Banda" className="w-full bg-black border-2 border-white p-3 font-bold uppercase text-sm text-white outline-none focus:border-yellow-400" value={newInterview.band_name} onChange={e => setNewInterview({...newInterview, band_name: e.target.value})} required />
              <div className="space-y-2 border-2 border-dashed border-zinc-700 p-4 text-center relative hover:border-yellow-400 transition-colors">
                <p className="text-xs font-black uppercase text-zinc-500">{uploading ? 'Subiendo...' : (newInterview.image_url ? 'Foto Lista ✅' : 'Subir Foto de la Banda')}</p>
                <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={async (e) => {
                  const file = e.target.files?.[0];
                  if (file) {
            ...
            <div className="space-y-4">
              <textarea placeholder="Contenido..." className="w-full h-40 bg-black border-2 border-white p-3 font-bold text-sm text-white outline-none focus:border-yellow-400" value={newInterview.content} onChange={e => setNewInterview({...newInterview, content: e.target.value})} required />
              <button type="submit" disabled={uploading} className="w-full bg-yellow-400 text-black font-black uppercase py-3 hover:bg-white disabled:opacity-50">Publicar Ahora</button>
            </div>
          </form>
        </section>
      </div>
